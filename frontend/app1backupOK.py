...
# Configuração do AWS AppConfig
APP_ID = os.environ.get('APPCONFIG_APP_ID')
ENV_ID = os.environ.get('APPCONFIG_ENV_ID')
CONFIG_ID = os.environ.get('APPCONFIG_CONFIG_ID')
BACKEND_URL = os.environ.get('BACKEND_URL', 'http://backend:5000')

def get_feature_flag():
    """Consulta o status da feature flag e o valor do desconto usando AWS AppConfig Data"""
    logger.info("Iniciando busca da feature flag de desconto no frontend")
    client = boto3.client('appconfigdata')
    
    try:
        logger.info(f"Iniciando sessão de configuração - App: {APP_ID}, Env: {ENV_ID}, Config: {CONFIG_ID}")
        ...
        logger.info("Buscando a última configuração")
        ...
        if content:
            ...
            logger.warning("Conteúdo da configuração vazio")
